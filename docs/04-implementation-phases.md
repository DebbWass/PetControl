...
## ספרינט 6 – ייצוא תיק רפואי ל-PDF (משוער: 5-7 שעות)

| # | משימה |
|---|---|
| 6.1 | התקנת `expo-print`, `expo-sharing`, `expo-file-system` + dev client חדש |
| 6.2 | `getRecords()` גנרי ב-`firestore.ts` + `medicalReportData.ts` (איסוף חד-פעמי) |
| 6.3 | `weightChartSvg.ts` – גרף משקל כ-SVG inline (טהור, בר-בדיקה) |
| 6.4 | `medicalReportHtml.ts` – מסמך A4 מלא: שער, משקל, תרופות, חיסונים, טיפולים, תורים, מזון, מסמכים, סיכום |
| 6.5 | `exportMedicalReport.ts` – תזמור + שיתוף + הדפסה + שם קובץ קריא |
| 6.6 | תפריט שלוש נקודות במסך פרופיל החיה + דיאלוג התקדמות/תוצאה |
| 6.7 | מפתחות i18n (`export.*`) בעברית ובאנגלית כולל צורות רבים |
| 6.8 | בדיקות יחידה לשני הבילדרים הטהורים |
| 6.9 | עדכון מסמכי תיעוד + STP/STR |

**הגדרת סיום:** ניתן לייצא את כל נתוני החיה ל-PDF בגודל A4, לשתף ולהדפיס, בעברית ובאנגלית.
...
| מודול נייטיבי חדש דורש dev client חדש | ודאית | בינונית | `require` עצל + הודעת `export.moduleMissing` במקום קריסה |
| RTL/עברית לא נכונים ב-WebView של ההדפסה | בינונית | בינונית | `dir` על `<html>`, `text-align: start`, בדיקה ידנית משתי השפות |
| PDF ענק לחיה עם היסטוריית מזון ארוכה | בינונית | נמוכה | מזון מוגבל ל-30 רשומות + ספירה כוללת |
